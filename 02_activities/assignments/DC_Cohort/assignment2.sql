...
SELECT market_date, 
SUM(quantity*cost_to_customer_per_qty) as totalperday
FROM customer_purchases
...
WITH rankmax AS (
	...
	SELECT market_date, totalperday,
	RANK() OVER(ORDER BY totalperday DESC) as [orderedtotal]
...
WITH rankmin AS (
	...
	SELECT market_date, totalperday,
	RANK() OVER(ORDER BY totalperday ASC) as [orderedtotal]
	FROM temp.customer_purchases
	)
SELECT * FROM rankmin
WHERE orderedtotal = 1
...
CREATE TABLE temp.rev AS
SELECT
v.vendor_name,
p.product_name,
(vi.original_price*5*c.customercount) AS totalmade
...
/*
DISCLAIMER I DID NOT DO THIS EXACTLY CORRECTLY. I GIVE UP - SPENT WAY TOO MUCH TIME TRYING TO FIGURE IT OUT, 
THE FINAL TABLE OUTPUT IS CORRECT BUT TABLE NAME IS NOT. SORRY :< IM SAD
